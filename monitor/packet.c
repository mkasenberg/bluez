					...
					uint8_t size)
{
	...
	print_field("Min Procedure Interval: %d", le16_to_cpu(cmd->min_procedure_interval));
	print_field("Max Procedure Interval: %d", le16_to_cpu(cmd->max_procedure_interval));
	print_field("Max Procedure Count: %d", le16_to_cpu(cmd->max_procedure_count));
	print_field("Min Subevent Len: %d", get_le24(cmd->min_subevent_len));
	print_field("Max Subevent Len: %d", get_le24(cmd->max_subevent_len));
	print_field("Tone Antenna Config Selection: %d", cmd->tone_antenna_config_selection);
...
static const struct opcode_data opcode_table[] = {
	...
	{ BT_HCI_CMD_LE_CS_SEC_ENABLE, BT_HCI_BIT_LE_CS_SEC_ENABLE,
				...
				true },
	{ BT_HCI_CMD_LE_CS_SET_DEF_SETTINGS, BT_HCI_BIT_LE_CS_SET_DEF_SETTINGS,
				"LE CS Set Default Settings",
				le_cs_set_def_settings_cmd,
				sizeof(struct bt_hci_cmd_le_cs_set_def_settings),
				true,
				le_cs_set_def_settings_rsp,
				sizeof(struct bt_hci_rsp_le_cs_set_def_settings),
				true },
	{ BT_HCI_CMD_LE_CS_RD_REM_FAE, BT_HCI_BIT_LE_CS_RD_REM_FAE,
				...
				true },
	{ BT_HCI_CMD_LE_CS_WR_CACHED_REM_FAE, BT_HCI_BIT_LE_CS_WR_CACHED_REM_FAE,
	...
	{ BT_HCI_CMD_LE_CS_CREATE_CONFIG, BT_HCI_BIT_LE_CS_CREATE_CONFIG,
				"LE CS Create Config", le_cs_create_config_cmd,
	...
	{ BT_HCI_CMD_LE_CS_SET_CHAN_CLASS, BT_HCI_BIT_LE_CS_SET_CHAN_CLASS,
				...
				true},
	{ BT_HCI_CMD_LE_CS_SET_PROC_PARAMS, BT_HCI_BIT_LE_CS_SET_PROC_PARAMS,
	...
	{ BT_HCI_CMD_LE_CS_PROC_ENABLE, BT_HCI_BIT_LE_CS_PROC_ENABLE,
				"LE CS Procedure Enable", le_cs_proc_enable_cmd,
...
static void le_cs_rd_rem_supp_cap_complete_evt(struct timeval *tv, uint16_t index,
					const void *data, uint8_t size)
{
	...
	print_field("Max Antenna Paths Supported: %d", evt->max_antenna_paths_supported);
					...
					const void *data, uint8_t size)
{
	...
	print_field("Subevent Interval: %d", le16_to_cpu(evt->subevent_interval));
	print_field("Event Interval: %d", le16_to_cpu(evt->event_interval));
	print_field("Procedure Interval: %d", le16_to_cpu(evt->procedure_interval));
	print_field("Procedure Count: %d", le16_to_cpu(evt->procedure_count));
	print_field("Max Procedure Len: %d", le16_to_cpu(evt->max_procedure_len));
}
...
static void print_cs_packet_quality(uint8_t packet_quality)
{
	...
	case 0x00:
		str = "CS Access Address check is successful, and all bits match "
...
static int print_cs_mode3_result(const uint8_t *data, uint8_t size)
{
	...
	tod_toa = get_le16(data + 3);
	if (tod_toa == 0x8000) {
		print_field("  ToA_ToD: Time difference is not available (0x%4.4x)",
					tod_toa);
	} else {
		print_field("  ToA_ToD: %d (0x%4.4x)", tod_toa, (int16_t)tod_toa);
	}
...
static void print_cs_procedure_done_status(uint8_t status)
{
	...
	case 0x00:
		...
		break;
	case 0x01:
		str = "Partial results with more to follow for the CS procedure";
		break;
	case 0x0F:
					...
					const void *data, uint8_t size)
{
	...
	print_hex_field("Steps data: ", evt->steps +
		sizeof(struct bt_hci_evt_le_cs_subevent_result), size -
